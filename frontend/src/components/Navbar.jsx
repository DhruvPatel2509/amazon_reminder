...
const links = [
  ...
  },
  {
    ...
    icon: (
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927l1.519 4.674a1 1 0 00.95.69h4.915l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888 1.518-4.674a1 1 0 00-.363-1.118L3.165 8.291h4.914a1 1 0 00.951-.69l1.519-4.674z" />
    ),
  },
  {
        ...
        <div className="flex min-h-16 items-center justify-between gap-3 py-2">
          ...
          <div className="hidden items-center gap-1 lg:flex">
            ...
            {notifications.length > 0 && (
              <Link to="/" className="relative ml-2 rounded-lg p-2 text-gray-300 transition-all hover:bg-card hover:text-white">
        ...
        {open && (
          <div className="border-t border-border py-3 lg:hidden">
            <div className="grid gap-2 sm:grid-cols-2">
              {links.map((link) => (
                <NavItem key={link.to} link={link} compact onClick={() => setOpen(false)} />
              ))}
              {notifications.length > 0 && (
                <Link
                  to="/"
                ...
                >
                  ...
                  <span className="rounded-full bg-danger px-2 py-0.5 text-xs font-bold text-white">{notifications.length}</span>
                </Link>
              )}
            </div>
          </div>
