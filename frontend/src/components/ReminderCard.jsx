  ...
  return (
    <div className="rounded-lg border border-border bg-surface/70 p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <span className={`inline-flex rounded-md border px-2 py-1 text-xs font-display font-semibold ${type.accent}`}>
            {type.label}
          </span>
        ...
        <div className="flex flex-col items-end gap-2">
          <span className={`status-badge border ${status.classes}`}>{status.label}</span>
      ...
      <div className="grid gap-5 2xl:grid-cols-[160px_1fr]">
        <div className="space-y-3">
          <div className="mx-auto aspect-square w-full max-w-[220px] overflow-hidden rounded-lg border border-border bg-surface flex items-center justify-center lg:max-w-none">
            {image ? (
              <img src={image} alt={`Product for order ${primary.orderId}`} className="h-full w-full object-contain bg-white" />
            ) : (
              <div className="px-4 text-center text-sm text-gray-300">Product image will appear after Amazon link is saved.</div>
            )}
          </div>
          {amazonLink && (
            <a
        ...
        <div>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-xs font-display uppercase tracking-widest text-gray-400">Order ID</p>
              <h3 className="mt-1 break-all font-display text-2xl font-bold text-white">#{primary.orderId}</h3>
            </div>
          ...
          <div className="space-y-3">
            {['review', 'refundForm', 'refund'].map((type) => {
              ...
              if (!reminder) {
                return (
                  <div key={type} className="rounded-lg border border-dashed border-border bg-surface/40 p-3 text-sm text-gray-400">
